...
bool UpgradeNetAsNeeded(const string& param_file, NetParameter* param) {
  ...
  if (NetNeedsV0ToV1Upgrade(*param)) {
    ...
    // upgrade it.
    cout << "Attempting to upgrade input file specified using deprecated "
              << "V0LayerParameter: " << param_file << endl;
    NetParameter original_param(*param);
    if (!UpgradeV0Net(original_param, param)) {
      success = false;
      cerr << "Warning: had one or more problems upgrading "
          << "V0NetParameter to NetParameter (see above); continuing anyway.";
    } else {
      cout << "Successfully upgraded file specified using deprecated "
                << "V0LayerParameter";
    }
    cout << "Note that future Caffe releases will not support "
  ...
  if (NetNeedsDataUpgrade(*param)) {
    ...
    cout << "Note that future Caffe releases will only support "
                 << "transform_param messages for transformation fields.";
  }
  if (NetNeedsV1ToV2Upgrade(*param)) {
    cout << "Attempting to upgrade input file specified using deprecated "
              << "V1LayerParameter: " << param_file << endl;
    NetParameter original_param(*param);
  ...
  if (NetNeedsBatchNormUpgrade(*param)) {
    cout << "Attempting to upgrade batch norm layers using deprecated "
              << "params: " << param_file;
    UpgradeNetBatchNorm(param);
    cout << "Successfully upgraded batch norm layers using deprecated "
              << "params.";
  }
  return success;
}
...
void ReadNetParamsFromTextFileOrDie(const string& param_file,
  ...
  if (!ReadProtoFromTextFile(param_file, param)) {
    cerr << "Failed to parse NetParameter file: " << param_file << endl;
    exit(1);
  }
  ...
  for (int i = 0; i < param.layers_size(); ++i) {
    ...
    for (int j = 0; j < layer_connection.bottom_size(); ++j) {
      ...
      if (blob_name_to_last_top_idx.find(blob_name) ==
        ...
        cerr << "Unknown blob input " << blob_name << " to layer " << j;
        exit(1);
      }
  ...
  if (v0_layer_connection.has_layer()) {
    ...
    if (v0_layer_param.has_num_output()) {
      if (type == "conv") {
        ...
        cerr << "Unknown parameter num_output for layer type " << type;
    ...
    if (v0_layer_param.has_biasterm()) {
      if (type == "conv") {
        ...
        cerr << "Unknown parameter biasterm for layer type " << type;
        is_fully_compatible = false;
      }
    }
    if (v0_layer_param.has_weight_filler()) {
      if (type == "conv") {
        ...
        cerr << "Unknown parameter weight_filler for layer type " << type;
        is_fully_compatible = false;
      }
    }
    if (v0_layer_param.has_bias_filler()) {
      if (type == "conv") {
        ...
        cerr << "Unknown parameter bias_filler for layer type " << type;
        is_fully_compatible = false;
      }
    }
    if (v0_layer_param.has_pad()) {
      if (type == "conv") {
        ...
        cerr << "Unknown parameter pad for layer type " << type;
        is_fully_compatible = false;
      }
    }
    if (v0_layer_param.has_kernelsize()) {
      if (type == "conv") {
        ...
        cerr << "Unknown parameter kernelsize for layer type " << type;
        is_fully_compatible = false;
      }
    }
    if (v0_layer_param.has_group()) {
      if (type == "conv") {
        ...
        cerr << "Unknown parameter group for layer type " << type;
        is_fully_compatible = false;
      }
    }
    if (v0_layer_param.has_stride()) {
      if (type == "conv") {
        ...
        cerr << "Unknown parameter stride for layer type " << type;
        is_fully_compatible = false;
      }
    }
    if (v0_layer_param.has_pool()) {
      if (type == "pool") {
        ...
        case V0LayerParameter_PoolMethod_STOCHASTIC:
          ...
          break;
        default:
          cerr << "Unknown pool method " << pool;
          is_fully_compatible = false;
        }
      } else {
        cerr << "Unknown parameter pool for layer type " << type;
        is_fully_compatible = false;
      }
    }
    if (v0_layer_param.has_dropout_ratio()) {
      if (type == "dropout") {
        ...
        cerr << "Unknown parameter dropout_ratio for layer type " << type;
        is_fully_compatible = false;
      }
    }
    if (v0_layer_param.has_local_size()) {
      if (type == "lrn") {
        ...
        cerr << "Unknown parameter local_size for layer type " << type;
        is_fully_compatible = false;
      }
    }
    if (v0_layer_param.has_alpha()) {
      if (type == "lrn") {
        ...
        cerr << "Unknown parameter alpha for layer type " << type;
        is_fully_compatible = false;
      }
    }
    if (v0_layer_param.has_beta()) {
      if (type == "lrn") {
        ...
        cerr << "Unknown parameter beta for layer type " << type;
        is_fully_compatible = false;
      }
    }
    if (v0_layer_param.has_k()) {
      if (type == "lrn") {
        ...
        cerr << "Unknown parameter k for layer type " << type;
        is_fully_compatible = false;
      }
    }
    if (v0_layer_param.has_source()) {
      if (type == "data") {
        ...
        cerr << "Unknown parameter source for layer type " << type;
        is_fully_compatible = false;
      }
    }
    ...
    if (v0_layer_param.has_batchsize()) {
      if (type == "data") {
        ...
        cerr << "Unknown parameter batchsize for layer type " << type;
        is_fully_compatible = false;
      }
    }
    ...
    if (v0_layer_param.has_rand_skip()) {
      if (type == "data") {
        ...
        cerr << "Unknown parameter rand_skip for layer type " << type;
        is_fully_compatible = false;
      }
    }
    if (v0_layer_param.has_shuffle_images()) {
      if (type == "images") {
        ...
        cerr << "Unknown parameter shuffle for layer type " << type;
        is_fully_compatible = false;
      }
    }
    if (v0_layer_param.has_new_height()) {
      if (type == "images") {
        ...
        cerr << "Unknown parameter new_height for layer type " << type;
        is_fully_compatible = false;
      }
    }
    if (v0_layer_param.has_new_width()) {
      if (type == "images") {
        ...
        cerr << "Unknown parameter new_width for layer type " << type;
        is_fully_compatible = false;
      }
    }
    if (v0_layer_param.has_concat_dim()) {
      if (type == "concat") {
        ...
        cerr << "Unknown parameter concat_dim for layer type " << type;
        is_fully_compatible = false;
      }
    }
    if (v0_layer_param.has_det_fg_threshold()) {
      if (type == "window_data") {
        ...
        cerr << "Unknown parameter det_fg_threshold for layer type "
                   << type;
        is_fully_compatible = false;
      }
    }
    if (v0_layer_param.has_det_bg_threshold()) {
      if (type == "window_data") {
        ...
        cerr << "Unknown parameter det_bg_threshold for layer type "
                   << type;
        is_fully_compatible = false;
      }
    }
    if (v0_layer_param.has_det_fg_fraction()) {
      if (type == "window_data") {
        ...
        cerr << "Unknown parameter det_fg_fraction for layer type "
                   << type;
        is_fully_compatible = false;
      }
    }
    if (v0_layer_param.has_det_context_pad()) {
      if (type == "window_data") {
        ...
        cerr << "Unknown parameter det_context_pad for layer type "
                   << type;
        is_fully_compatible = false;
      }
    }
    if (v0_layer_param.has_det_crop_mode()) {
      if (type == "window_data") {
        ...
        cerr << "Unknown parameter det_crop_mode for layer type "
                   << type;
        is_fully_compatible = false;
      }
    }
    if (v0_layer_param.has_hdf5_output_param()) {
      if (type == "hdf5_output") {
        ...
        cerr << "Unknown parameter hdf5_output_param for layer type "
                   << type;
        is_fully_compatible = false;
      }
    }
...
V1LayerParameter_LayerType UpgradeV0LayerType(const string& type) {
  if (type == "accuracy") {
    ...
    cerr << "Unknown layer name: " << type << endl;
    exit(1);
...
bool UpgradeV1Net(const NetParameter& v1_net_param, NetParameter* net_param) {
  if (v1_net_param.layer_size() > 0) {
    cout << "Refusing to upgrade inconsistent NetParameter input; "
  ...
  for (int i = 0; i < v1_net_param.layers_size(); ++i) {
    if (!UpgradeV1LayerParameter(v1_net_param.layers(i),
                                 net_param->add_layer())) {
      cerr << "Upgrade of input layer " << i << " failed.";
      is_fully_compatible = false;
    }
  }
  return is_fully_compatible;
...
bool UpgradeV1LayerParameter(const V1LayerParameter& v1_layer_param,
  ...
  for (int i = 0; i < v1_layer_param.blob_share_mode_size(); ++i) {
    ...
    case V1LayerParameter_DimCheckMode_PERMISSIVE:
      ...
      break;
    default:
      cerr << "Unknown blob_share_mode: "
                 << v1_layer_param.blob_share_mode(i) << endl;
      exit(1);
      break;
    }
  ...
  if (v1_layer_param.has_layer()) {
    cerr << "Input NetParameter has V0 layer -- ignoring.";
    is_fully_compatible = false;
  }
  return is_fully_compatible;
}
...
const char* UpgradeV1LayerType(const V1LayerParameter_LayerType type) {
  ...
  case V1LayerParameter_LayerType_THRESHOLD:
    return "Threshold";
  default:
    cerr << "Unknown V1LayerParameter layer type: " << type << endl;
    return "";
  }
}
...
bool UpgradeSolverType(SolverParameter* solver_param) {
  ...
  if (solver_param->has_solver_type()) {
    ...
    case SolverParameter_SolverType_ADAM:
      ...
      break;
    default:
      cerr << "Unknown SolverParameter solver_type: " << type << endl;
    }
    solver_param->set_type(type);
    solver_param->clear_solver_type();
  } else {
    cerr << "Warning: solver type already up to date. ";
    return false;
  }
  return true;
}
